...
from ..formstotal.modelo import ModeloForm
...
class JsonView(View):
    def get(self, request):
        ...
        if order == 'desc':
            ...
            order = str('')
        contact_list = Modelo.objects.all().order_by("-id")
        if len(sort)>0:
            contact_list = Modelo.objects.all().order_by(order+sort)
        if len(search)>0:
            if len(sort) == 0:
                sort='id'
            contact_list = Modelo.objects.filter(
        ...
        for objeto in contacts:
            ...
            dicobjec['marca__nombre'] = objeto.marca.nombre
...
@method_decorator(login_required, name='dispatch')
class FormView(View):
    template_name = 'modelo/formulario.html'
    ...
    def get(self, request, *args, **kwargs):
        ...
        if ids == 0:
            ...
            objeto = Modelo.objects.get(pk=ids)
...
class EliminarView(DeleteView):
    model = Modelo
    template_name = 'modelo/eliminar_formulario.html'
